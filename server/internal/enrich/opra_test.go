...
not json at all
{"type":"eq","id":"e1","data":{"author":"oratory1990","type":"parametric_eq","product_id":"senn::hd650","parameters":{"gain_db":-6.8,"bands":[{"type":"peak_dip","frequency":105,"gain_db":3.1,"q":0.7},{"type":"low_pass","frequency":18000,"gain_db":0,"slope":12}]}}}
{"type":"product","id":"senn::noeq","data":{"name":"No EQ","vendor_id":"senn"}}`
	...
	eq := p.Eqs[0]
	if eq.Author != "oratory1990" || eq.GainDB != -6.8 || len(eq.Bands) != 2 {
		t.Fatalf("eq = %+v", eq)
	}
